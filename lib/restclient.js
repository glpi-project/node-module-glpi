...
class GlpiRestClient {
	...
	killSession() {
		return new Promise((resolve, reject) => {
			try {
				if (!this.sessionToken) return 'Uninitialized session'
				this._prepareRequest('GET', 'killSession', (response, data) => {
	...
	getMyProfiles() {
		return new Promise((resolve, reject) => {
			try {
				if (!this.sessionToken) return 'Uninitialized session'
				this._prepareRequest('GET', 'getMyProfiles', (response, data) => {
					resolve({
						status: response.statusCode,
						data: data
					})
	...
	getActiveProfile() {
		return new Promise((resolve, reject) => {
			try {
				if (!this.sessionToken) return 'Uninitialized session'
				this._prepareRequest('GET', 'getActiveProfile', (response, data) => {
					resolve({
						status: response.statusCode,
						data: data
					})

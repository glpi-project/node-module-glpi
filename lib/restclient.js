...
const GETANITEMQUERY = require('./itemtype')
...
class GlpiRestClient {
	...
	changeActiveEntities (entities_id, is_recursive) {
		return new Promise((resolve, reject) => {
			try {
				if (!this.sessionToken) throw this._requestException(401, {errorCode: '', errorMessage: 'Uninitialized session'})
				let options = {"activeEntities": {}}
				if (entities_id) {
					if (!this._isNumber(entities_id)) throw this._requestException(400, {errorCode: '', errorMessage: 'Invalid entities_id'})
					if (entities_id < 0 ) throw this._requestException(400, {errorCode: '', errorMessage: 'Invalid entities_id'})
					options = {"activeEntities": entities_id}
				}
				if (is_recursive) {
					if(typeof(is_recursive) === "boolean") {
						options = {"activeEntities": {...options.activeEntities, is_recursive}}
					} else {
	...
	passwordReset (email, password_forget_token, password) {
		return new Promise((resolve, reject) => {
			try {
				const options = {
					"lostPassword": {
